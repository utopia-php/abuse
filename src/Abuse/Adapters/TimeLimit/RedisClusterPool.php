...
class RedisClusterPool extends TimeLimit
{
    /**
     * @var int
     */
    ...
    /**
     * @param UtopiaPool<\RedisCluster> $pool
     */
    ...
    protected function count(string $key, int $timestamp): int
    {
        ...
        /** @var int $count */
        $count = $this->useRedis(function (\RedisCluster $redis) use ($key, $timestamp): int {
            $count = $redis->get(RedisCluster::NAMESPACE . '__' . $key . '__' . $timestamp);
    ...
    protected function hit(string $key, int $timestamp): void
    {
        ...
        $ttl = $this->ttl;
        $key = RedisCluster::NAMESPACE . '__' . $key . '__' . $timestamp;

        $this->useRedis(function (\RedisCluster $redis) use ($key, $ttl): void {
            $redis->multi();
            $redis->incr($key);
    ...
    protected function set(string $key, int $timestamp, int $value): void
    {
        ...
        $this->useRedis(function (\RedisCluster $redis) use ($key, $ttl, $value): void {
            $redis->multi();
    ...
    public function getLogs(?int $offset = null, ?int $limit = 25): array
    {
        ...
        $result = $this->useRedis(function (\RedisCluster $redis) use ($offset, $limit): array {
            $matches = [];
            $pattern = RedisCluster::NAMESPACE . '__*';
    ...
    private function discard(\RedisCluster $redis): void
    {
        try {
            $redis->discard();
        } catch (Throwable) {
