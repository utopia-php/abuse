...
- `utopia-php/database` is required as `8.0.*` instead of a development branch pinned through an inline alias, and the `repositories` block is gone. A `repositories` entry is honoured only in the root package, so consumers could not resolve the previous requirement without replicating it themselves.
